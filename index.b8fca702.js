...
class Game {
    ...
    move(direction) {
        ...
        switch(direction){
            case "left":
                for(let i = 0; i < size; i++)board[i] = this.mergeRow(board[i]);
                break;
            case "right":
                for(let i = 0; i < size; i++)board[i] = this.mergeRow(board[i].reverse()).reverse();
                break;
            case "up":
                for(let col = 0; col < size; col++){
                    const column = board.map((row)=>row[col]);
                    const mergedColumn = this.mergeRow(column);
                    for(let row = 0; row < size; row++)board[row][col] = mergedColumn[row];
                }
                break;
            case "down":
                for(let col = 0; col < size; col++){
                    ...
                    const mergedColumn = this.mergeRow(column);
                    for(let row = 0; row < size; row++)board[row][col] = mergedColumn[size - row - 1];
                }
                break;
        }
        if (this.hasMoved) {
    ...
    mergeRow(row) {
        ...
        for(let i = 0; i < row.length; i++){
            ...
            if (i + 1 < row.length && row[i] === row[i + 1]) {
                ...
                if (mergedRow.length - 1 !== i) moved = true;
            }
        }
        while(mergedRow.length < row.length)mergedRow.push(0);
        if (moved) this.hasMoved = true;
        return mergedRow;
    }
    addRandomTile() {
        const emptyCells = [];
        for(let i = 0; i < 4; i++){
            for(let j = 0; j < 4; j++)if (this.board[i] && this.board[i][j] === 0) emptyCells.push([
                i,
                j
            ]);
        }
        if (emptyCells.length > 0) {
            ...
            if (this.board[row] && this.board[row][col] === 0) this.board[row][col] = Math.random() < 0.1 ? 4 : 2;
        }
    }
    checkWinLose() {
        if (this.isWin()) {
    ...
    canMove() {
        for(let i = 0; i < 4; i++)for(let j = 0; j < 4; j++){
            ...
            if (i > 0 && this.board[i][j] === this.board[i - 1][j]) return true;
            if (i < 3 && this.board[i][j] === this.board[i + 1][j]) return true;
            if (j > 0 && this.board[i][j] === this.board[i][j - 1]) return true;
